    ...
    >
        ...
        <Box sx={{display: 'flex', flexDirection: 'column', overflowY: 'auto'}}>
            {props.messages.map((item, index) =>
                ...
                >
                    {item.count > 1 && <span
                        style={{marginRight: 1}}>({item.count})</span>} {item.text?.length > 70 ? item.text.slice(0, 68) + '...' : item.text}
                </MenuItem>)}
        </Box>
...
export function Composer(props: {
    ...
    async function loadAndAttachFiles(files: FileList, overrideFileNames: string[]) {
        ...
        if (newText.length && props.conversationId) {
            setComposeText('');
            props.onSendMessage(props.conversationId, newText);
            appendSentMessage(newText);
        }
    }
    ...
    const chatButton = (
        <Button
            fullWidth variant={isWriteUser ? 'soft' : 'solid'}
            ...
            endDecorator={isWriteUser ? <SendIcon sx={{fontSize: 18}}/> : isReAct ? <PsychologyIcon/> : isEmbeddings ?
                <LocalLibraryIcon/> : <TelegramIcon/>}
        >
            {isWriteUser ? 'Write' : isReAct ? 'ReAct' : isFollowUp ? 'Chat+' : isEmbeddings ? 'LawLoom' : 'GPT-4'}
        </Button>
    );
                ...
                <Grid xs={12} md={9}><Stack direction='row' spacing={{xs: 1, md: 2}}>
                    ...
                    <Box sx={{display: 'flex', flexDirection: 'column', gap: {xs: 0, md: 2}}}>
                        ...
                        <Tooltip
                            ...
                            <Button fullWidth variant='plain' color='neutral' onClick={handleShowFilePicker}
                        ...
                        <input type='file' multiple hidden ref={attachmentFileInputRef}
                    ...
                    <Box sx={{flexGrow: 1, position: 'relative'}}>
                        ...
                        <Box sx={{position: 'relative'}}>
                            ...
                            <TokenProgressbar history={historyTokens} response={responseTokens} direct={directTokens}
                                              limit={tokenLimit}/>}
                        ...
                        {!!tokenLimit &&
                        <TokenBadge directTokens={directTokens} indirectTokens={historyTokens + responseTokens}
                                    tokenLimit={tokenLimit} absoluteBottomRight/>}
                    ...
                    <Stack spacing={2}>
                        ...
                        <Box sx={{display: 'flex', flexDirection: 'row'}}>
                            ...
                            {sentMessages.length > 0 && (
                                <IconButton disabled={!!sentMessagesAnchor} onClick={showSentMessages}
                        ...
                        <Stack direction='row' spacing={1} sx={{
                            ...
                            {sentMessages.length > 0 && (
                                <Button disabled={!!sentMessagesAnchor} fullWidth variant='plain' color='neutral'
                                        startDecorator={<KeyboardArrowUpIcon/>} onClick={showSentMessages}>
                                    History
                                </Button>
                            )}
                        </Stack>
