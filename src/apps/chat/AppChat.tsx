...
import { runEmbeddingsUpdatingState } from './editors/agi-embeddings';
...
export function AppChat() {
    ...
    const handleExecuteConversation = async (chatModeId: ChatModeId, conversationId: string, history: DMessage[]) => {
        ...
        if (chatModeId && chatLLMId && systemPurposeId) {
            switch (chatModeId) {
                ...
                case 'embeddings':
                    if (lastMessage?.text) {
                        return await runEmbeddingsUpdatingState(conversationId, history, lastMessage.text, chatLLMId, systemPurposeId);
                    }
            }
        }
    ...
    return <>
        ...
        <FlattenerModal conversationId={flattenConversationId} onClose={() => setFlattenConversationId(null)}/>}
